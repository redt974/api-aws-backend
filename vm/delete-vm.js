...
router.post("/delete", async (req, res) => {
    ...
    try {
        ...
        if (vm.rowCount === 0) return res.status(404).send("VM non trouvée.");

        exec("cd terraform && terraform destroy -auto-approve", async (err) => {
            if (err) {
                console.error(`Erreur Terraform : ${err.message}`);
                return res.status(500).send("Erreur lors de la suppression de la VM.");
            }

            await pool.query("DELETE FROM vms WHERE id = $1", [vmId]);
            res.send({ message: "VM supprimée avec succès." });
        });
    } catch (err) {
